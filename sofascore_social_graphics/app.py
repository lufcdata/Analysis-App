...
from parsers import build_metric_leader_rows, build_player_stat_rows, extract_match_statistics, extract_players, parse_match_info
...
payload=st.session_state.match_payload; match=parse_match_info(payload["basic"]); match_rows=extract_match_statistics(payload["statistics"]); players=extract_players(payload["lineups"],match)
...
if graphic_type=="Match Statistics":
    ...
    with preview_col: st.image(png,caption="1080 × 1350 preview",width=540)
    with action_col:
        st.subheader("Match Statistics"); st.caption("Full-time SofaScore performance numbers for both teams.")
        st.download_button("Download PNG",data=png,file_name=f"{match.event_id}_match_stats.png",mime="image/png",type="primary",width="stretch")
    ...
    with control_col:
        ...
        png=render_player_graphic(player.name,player.opponent,rows,minutes,team=player.team,match=match)
